...
//        });
        ...
        // "onlyMine" is set when a rider opens this screen from their own "My Ride Requests"
        // button, to review/update/delete their own unaccepted ride requests.
        boolean onlyMine = getIntent().getBooleanExtra("onlyMine", false);
        Query query = myRef;
        if (onlyMine) {
            FirebaseUser currentUser = FirebaseAuth.getInstance().getCurrentUser();
            String currentUserEmail = currentUser != null ? currentUser.getEmail() : null;
            query = myRef.orderByChild("riderName").equalTo(currentUserEmail);
        }
        ...
        query.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                noDataTextView.setVisibility(rideRequestsList.isEmpty() ? View.VISIBLE : View.GONE);
            }
